...
- `background` — the **canvas** color (19.10). `SVGRenderer` applies it as the inline background of the `<svg>`, and the standalone export uses it too. In the editor it also **decides the theme base**: with a dark canvas declared, the editor picks the dark base even if its chrome is light, because what must be legible is the text over YOUR background. **Without it the look is only half-done outside the editor**: `ygg render` painted white or the default dark depending on the `--dark` flag rather than on the file, and a gothic theme on a white page is not the gothic theme.
...
- `sizes.labelMinRadius` — minimum radius to carry **painted text**. This is the legibility tool at scale: a three-word name is about 157 viewBox units wide and an atlas has spacings around 60, so past a certain density labels cannot sit beside their nodes however far the layout separates them. Raising it leaves text on the landmarks only; **the name is not lost**: the node emits a `<title>` with the full name (native hover tooltip and the SVG accessible name) and, when interactive, the `aria-label` too.

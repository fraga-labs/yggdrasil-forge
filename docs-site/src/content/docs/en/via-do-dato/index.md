...
| `atlas-de-fisterra.json` | The **scale** example: 97 nodes and 174 edges across 6 shires, with NO positions and no bounds in the file — the engine places them with `mesh`, the only layout that reads the edges. Each shire is a closed WEB (ring, chords, keystones on opposite sides), and the file comes out of a reproducible generator. A generator need not know geometry. |

...
### `atlas-de-fisterra.json` — the SCALE example (19.3, rebuilt in 19.10)

*"The Finisterre Atlas"* — 97 nodes and 174 edges in **6 tinted regions**
around a single root, script-generated by
[`tools/galeria/atlas-fisterra.mjs`](../../tools/galeria/atlas-fisterra.mjs)
and never hand-edited. This is the answer to "does it hold up at size?",
and the tree to reach for when you want to see the renderer under load.
Exercises:

- **A real layout engine instead of hand-placed coordinates**: the
  document carries **no `position` and no `coordinateBounds`** and
  declares `layout: mesh` (`spacing`, `seed`, `curve: "arc"`,
  `centerGroupId`). The engine places all 97 nodes and the renderer
  derives the bounds. A generator does not need to know geometry.
- **A web, not a star.** Each shire is a closed mesh: a ring of twelve
  small nodes, chords every third one, a gateway wired to one in four,
  two keystones anchored on **opposite** sides of the ring, and an
  ascendancy above both. Regions are joined to their ring neighbours, so
  the six read as one tissue rather than six islands. `mesh` is the only
  layout that **reads the edges**, which is what keeps those edges short.
...
- The full `atlas` preset spec (including its own `background`, so
  `ygg render` without `--dark` already looks right), region tints
  included.
- The `ascendancy` node type (octagon), `keystone` gates, and two
  resources where the scarce one (`sona`) is only spent on keystones and
  ascendancies.

**Only seven nodes carry painted text** — the root and the six
ascendancies — because the preset sets `sizes.labelMinRadius: 40`. That
is measured, not taste: a name like *"Mestre de Temporais"* is about 157
